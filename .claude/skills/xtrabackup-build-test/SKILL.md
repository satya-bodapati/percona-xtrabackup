...
**Recreate the containers before every keyring/xbcloud run — never reuse them
across runs.** KMIP keeps the keys earlier runs created (no per-test object
group yet: `KMIP_OBJECT_GROUP` is commented out in `inc/keyring_kmip.sh`,
blocked on PS-9799), MinIO keeps buckets/IAM users, and Vault regenerates its
dev root token on each start. A stale key makes a test fail with
`xtrabackup succeeded when it was expected to fail` — looks like a product
bug, is not one.

On the epyc lab hosts this is all scripted:

```bash
bash /bigdisk_kioxia/satya/pxb-testenv/reset-services.sh   # teardown + reinit
. /bigdisk_kioxia/satya/pxb-testenv/env.sh                 # exports + health line
./run.sh -f -j 128
```

Two sourcing traps:
- **Source `env.sh` directly, never through a pipe** — `. env.sh | tail -1`
  runs in a subshell and every export is lost, silently falling back to the
  framework defaults.
- Those defaults are `127.0.0.1` and `/etc/pykmip/*`. Certs left there from a
  different image fail with
  `BIO_do_connect failed: ... tlsv1 alert decrypt error`. Keep `/etc/pykmip`
  in sync with the running container (reset-services.sh does) or always export
  the `KMIP_*` vars; compare with `openssl x509 -noout -fingerprint`.
