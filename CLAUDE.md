...
**Recreate the containers before every keyring/xbcloud run — do not reuse
them across runs.** `reset-services.sh` next to `env.sh` does the whole
teardown/reinit and re-copies the credentials:

```bash
bash /bigdisk_kioxia/satya/pxb-testenv/reset-services.sh
. /bigdisk_kioxia/satya/pxb-testenv/env.sh
./run.sh -f -j 128
```

Reuse is not safe, because the services keep state that the tests assume is
fresh:

- **KMIP keeps the keys earlier runs created.** There is no per-test object
  group yet — `KMIP_OBJECT_GROUP` is commented out in `inc/keyring_kmip.sh`,
  blocked on PS-9799 — so all tests share one key namespace. A test that
  expects a key to be gone then finds the stale one, and fails with
  `xtrabackup succeeded when it was expected to fail`. That reads like a
  product bug and is not one.
- **MinIO keeps buckets and IAM users** created by earlier xbcloud tests.
- **Vault's dev root token is regenerated on every start**, so a token
  captured from an earlier container is dead.

Two sourcing traps worth knowing:

- **Source `env.sh` directly, never through a pipe.** `. env.sh | tail -1`
  runs it in a subshell, so every `export` is lost and the run silently falls
  back to the framework defaults.
- **The defaults are `127.0.0.1` and `/etc/pykmip/*`** (see
  `inc/keyring_kmip.sh`). If the `KMIP_*` variables are not exported, the
  framework uses whatever certs happen to be in `/etc/pykmip` — and a copy
  left over from a different image fails with
  `BIO_do_connect failed: error:0A00041B:SSL routines::tlsv1 alert decrypt error`.
  Keep `/etc/pykmip` in sync with the running container (reset-services.sh
  does), or always export the variables. Compare fingerprints when in doubt:
