...
class logger {
  ...
  */
  void log_event(std::string msg, const char* const module);

  /** Constructor.
  @param[in]	level		Logging level
  @param[in]	err		Error message code. */
  logger(loglevel level, int err, const char *module)
      : m_err(err), m_level(level), m_module(module) {
  ...
  @param[in]	args		Variable length argument list */
  template <class... Args>
  explicit logger(loglevel level, int err, const char* module, Args &&... args)
      : m_err(err), m_level(level), m_module(module) {
    m_oss << msg(err, std::forward<Args>(args)...);
  }
  ...
  explicit logger(loglevel level, const char *module)
      : m_err(ER_IB_MSG_0), m_level(level), m_module(module) {}
...
class fatal_or_error : public logger {
 public:
  ...
  fatal_or_error(bool fatal) : logger(ERROR_LEVEL, INNO), m_fatal(fatal) {}
  ...
  explicit fatal_or_error(bool fatal, int err, Args &&... args)
      : logger(ERROR_LEVEL, err, INNO, std::forward<Args>(args)...), m_fatal(fatal) {}
...
class fatal_or_error : public ib::logger {
 public:
  ...
  /** Default constructor uses ER_IB_MSG_0
  @param[in]	fatal		true if it's a fatal message */
  fatal_or_error(bool fatal) : logger(ERROR_LEVEL, PXB), m_fatal(fatal) {}
  ...
  template <class... Args>
  explicit fatal_or_error(bool fatal, int err, Args &&... args)
      : logger(ERROR_LEVEL, err, PXB, std::forward<Args>(args)...), m_fatal(fatal) {}
