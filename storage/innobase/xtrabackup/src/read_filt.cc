...
static void rf_page_tracking_init(xb_read_filt_ctxt_t *ctxt,
  ...
  /* The combine distance is a byte count - the read request cost
  measured single-threaded at backup start (or the conservative
  fallback) in auto mode, the pinned --page-tracking-combine-distance
  otherwise. Converting it to pages of this tablespace's physical page
  size makes compressed tablespaces (zip size 1K-8K) combine across the
  same byte distance: combine every gap cheaper than one saved read. */
  const uint64_t distance = opt_page_tracking_combine_distance_auto
                                ? xb_read_request_cost
                                : opt_page_tracking_combine_distance;
  ctxt->max_gap = static_cast<ulint>(distance / ctxt->page_size);
...
static void rf_page_tracking_deinit(xb_fil_cur_t *cursor) {
  ...
  xb::info() << std::fixed << "pagetracking: " << cursor->rel_path << ": "
             ...
             << " ranges, typically " << std::setprecision(1) << avg_gap
             << " pages (" << avg_gap_bytes / 1024
             << "KB) apart; combine-distance=" << distance / 1024 << "KB"
             << (opt_page_tracking_combine_distance_auto ? " (auto)" : "")
             << " = " << ctxt->max_gap << " pages; combined them into "
             << ctxt->stat_groups << " reads: request reduction " << reduction
  ...
  typical distance between changed pages costs more than one read
  request, combining achieves little and the reads stay individual.
  Only when that distance is within reach of a plausible cost: past the
  clamp ceiling no calibration could ever combine across it - that is
  genuinely sparse data. The suggested pin is the next whole page past
  the typical distance, so it would combine the typical case. */
  if (opt_page_tracking_combine_distance_auto && reduction < 1.5 &&
      avg_gap_bytes > xb_read_request_cost &&
      avg_gap_bytes <= pagetracking::READ_REQUEST_COST_MAX_BYTES) {
    const uint64_t suggested_kb =
        static_cast<uint64_t>(avg_gap + 1.0) * ctxt->page_size / 1024;
    xb::info() << std::fixed << std::setprecision(1)
               << "pagetracking: " << cursor->rel_path
               << ": the changed data here is typically " << avg_gap
               << " pages (" << avg_gap_bytes / 1024
               << "KB) apart, more than one read request costs ("
               << xb_read_request_cost / 1024
               << "KB); reads stay individual - if sequential read "
                  "throughput is high, --page-tracking-combine-distance="
               << suggested_kb << "K may be faster";
  }
}
