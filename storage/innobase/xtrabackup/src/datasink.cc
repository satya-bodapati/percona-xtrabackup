...
ds_file_t *ds_open(ds_ctxt_t *ctxt, const char *path, MY_STAT *stat) {
  ...
  if (file != NULL) {
    ...
    /* Back-pointer to the owning ctxt so the framework can reach the
    pipeline this file belongs to (used for diagnostics and for any
    future per-pipeline state).  ds_file_t carries its own metrics
    binding -- propagated here from ctxt->metrics -- so wrappers can
    open inner pass-through files via ds_open_internal() that do not
    re-fire the counter on the same logical bytes. */
    file->ctxt = ctxt;
...
ds_file_t *ds_open_internal(ds_ctxt_t *ctxt, const char *path, MY_STAT *stat) {
  ...
  if (file != NULL) {
    /* Wrapper-internal opens must not propagate metrics: a wrapper that
    passes a write down to its inner dest_file would otherwise double-
    count when the inner ctxt is itself a metrics-bound head (the
    canonical example is ds_data's compressed leaf coinciding with
    ds_uncompressed_data, both pointing at the shared ds_local ctxt). */
