...
bool xb_crypt_read_key_file(const char *filename, void **key, uint *keylength) {
  ...
  if (!(fp = my_fopen(filename, O_RDONLY, MYF(0)))) {
    msg("%s:%s: unable to open config file \"%s\", errno(%d)\n", my_progname,
        __FUNCTION__, filename, my_errno());
    return false;
  }
...
gcry_error_t xb_crypt_init(uint *iv_len) {
  ...
  /* Now set up the key */
  if (ds_encrypt_key == NULL && ds_encrypt_key_file == NULL) {
...
void xb_crypt_cipher_close(gcry_cipher_hd_t cipher_handle) {
  if (encrypt_algo != GCRY_CIPHER_NONE) gcry_cipher_close(cipher_handle);
}
...
gcry_error_t xb_crypt_encrypt(gcry_cipher_hd_t cipher_handle, const uchar *from,
  ...
  gcry_md_hash_buffer(XB_CRYPT_HASH, to + from_len, from, from_len);
