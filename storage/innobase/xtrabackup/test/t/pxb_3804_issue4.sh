...
[ "$fsize" -ge "$need" ] || \
  die "Issue 4: $IBD is only $fsize bytes; need >= $need"

base=$(( page_size * page_no ))
vlog "Apply multi-field corruption to root page $page_no"
# PAGE_LEVEL (offset 90) = 99  -> 0x0063 (big-endian, 2 bytes)
printf '\x00\x63'         | dd of="$IBD" bs=1 seek=$(( base + 90 )) count=2 conv=notrunc
# PAGE_N_RECS (offset 86) = 0xFFFF
printf '\xFF\xFF'         | dd of="$IBD" bs=1 seek=$(( base + 86 )) count=2 conv=notrunc
# FIL_PAGE_NEXT (offset 12) = 0
printf '\x00\x00\x00\x00' | dd of="$IBD" bs=1 seek=$(( base + 12 )) count=4 conv=notrunc
...
if grep -qiE "Assertion failure|got signal|intentionally generate a memory trap" \
  ...
  die "Issue 4: xtrabackup CRASHED on multi-field root corruption instead of reporting corruption (PXB-3804)"
fi

grep -q "Starting table checks" $topdir/check.log || \
  die "Issue 4: check-tables did not start"
grep -q "is corrupted" $topdir/check.log || \
  die "Issue 4: corruption not reported"
grep -q "Table check failed" $topdir/check.log || \
  die "Issue 4: 'Table check failed' message not found"

vlog "Issue 4 passed: multi-field root corruption reported gracefully, no crash"
