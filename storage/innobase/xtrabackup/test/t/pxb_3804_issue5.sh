...
[ "$fsize" -ge "$need" ] || \
  die "Issue 5: $IBD is only $fsize bytes; need >= $need"
...
if grep -qiE "Assertion failure|got signal|intentionally generate a memory trap" \
  ...
  die "Issue 5: xtrabackup CRASHED in SDI/dictionary load instead of reporting corruption (PXB-3804)"
fi

grep -q "Starting table checks" $topdir/check.log || \
  die "Issue 5: check-tables did not start"
grep -qiE "is corrupted|corrupt|cannot load|Table check failed" $topdir/check.log || \
  die "Issue 5: corruption not reported"
grep -q "Table check failed" $topdir/check.log || \
  die "Issue 5: 'Table check failed' message not found"

vlog "Issue 5 passed: corrupt SDI PAGE_INDEX_ID reported gracefully, no crash"
