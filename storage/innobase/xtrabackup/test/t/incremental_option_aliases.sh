...
if grep -q "is deprecated and will be removed" $OUTFILE ; then
	die "the new option names must not emit a deprecation warning"
fi
...
xtrabackup --prepare --apply-log-only --target-dir=$topdir/full2
