############################################################################
# Test backup_size / uncompressed_size reporting in xtrabackup_info.
#
...
#   Invariant B  (--compress ):  uncompressed_size == sum_file_bytes(decompressed)
#
...
# then check uncompressed_size.  uncompressed_size is sampled from the
...
assert_decompressed_strict() {
  ...
  local total=$(sum_file_bytes "$dir")
  assert_eq "$us" "$total" \
    "$label B: uncompressed_size($us) == sum_file_bytes(decompressed $dir)"
}
...
assert_no_field "$topdir/lsn1/xtrabackup_info" uncompressed_size
...
assert_no_field "$topdir/lsn2/xtrabackup_info" uncompressed_size
...
us3=$(get_field "$topdir/lsn3/xtrabackup_info" uncompressed_size)
assert_positive "$bs3" "scen3 backup_size"
assert_positive "$us3" "scen3 uncompressed_size"
...
assert_no_field "$topdir/lsn4/xtrabackup_info" uncompressed_size
...
us5=$(get_field "$topdir/lsn5/xtrabackup_info" uncompressed_size)
assert_positive "$bs5" "scen5 backup_size"
assert_positive "$us5" "scen5 uncompressed_size"
...
assert_no_field "$topdir/lsn6/xtrabackup_info" uncompressed_size
...
us7=$(get_field "$topdir/lsn7/xtrabackup_info" uncompressed_size)
assert_positive "$bs7" "scen7 backup_size"
assert_positive "$us7" "scen7 uncompressed_size"
...
us10full=$(get_field "$topdir/lsn10full/xtrabackup_info" uncompressed_size)
assert_positive "$us10full" "scen10 full uncompressed_size"
...
us10inc1=$(get_field "$topdir/lsn10inc1/xtrabackup_info" uncompressed_size)
assert_positive "$us10inc1" "scen10 inc1 uncompressed_size"
...
if test -f $(dirname ${MYSQLD})/../lib/plugin/ha_rocksdb.so ; then
  ...
  us11=$(get_field "$topdir/lsn11/xtrabackup_info" uncompressed_size)
  assert_positive "$us11" "scen11 uncompressed_size"
...
us12=$(get_field "$topdir/lsn12/xtrabackup_info" uncompressed_size)
assert_positive "$us12" "scen12 uncompressed_size"
...
us13=$(get_field "$topdir/lsn13/xtrabackup_info" uncompressed_size)
assert_positive "$us13" "scen13 uncompressed_size"
...
if grep -q 'PUNCH HOLE support not available' $MYSQLD_ERRFILE ; then
  ...
  assert_no_field "$topdir/lsn14/xtrabackup_info" uncompressed_size
  ...
  us15=$(get_field "$topdir/lsn15/xtrabackup_info" uncompressed_size)
  assert_positive "$us15" "scen15 uncompressed_size"

  # Invariant B: decompress and check uncompressed_size against the
