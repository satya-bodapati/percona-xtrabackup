############################################################################
# Test backup size reporting with --lock-ddl=reduced and concurrent TRUNCATE.
#
# Uses debug sync to pause backup, TRUNCATE a table (creating a 0-size
# tablespace scenario), then resume. Verifies ds_statistics handles
# 0-size files gracefully and size reporting remains correct.
#
...
size_u=$(get_backup_size "$topdir/lsndir_reduced/xtrabackup_info")
assert_positive "$size_u" "reduced-lock backup_size"

if grep -q "^uncompressed_size = " "$topdir/lsndir_reduced/xtrabackup_info" ; then
  die "uncompressed_size should not be present without --compress"
fi

grep -q "Backup size:" $topdir/backup_reduced.log || die "Expected 'Backup size:' in log"

vlog "Size check passed: backup_size=$size_u"
