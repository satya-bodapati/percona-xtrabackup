###############################################################################
# PXB-3808: Atomic two-table swap (RENAME TABLE t1 TO __t, t2 TO t1, __t TO t2)
# inside a --lock-ddl=REDUCED backup window.
#
# The swap produces two .ren markers whose source/destination filenames form a
# cycle (t1<->t2). Processing them one-by-one fails because each destination
# file still exists when the other rename is applied. prepare must resolve the
# cycle via temporary staging names and end up with the rows swapped.
###############################################################################
...
# Distinct row counts so a silent overwrite produces a wrong row count instead
# of an undetectable permutation.
