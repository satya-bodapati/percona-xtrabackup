############################################################################
# PXB-3862: --page-tracking-combine-distance groups changed pages into read ranges
#
# Plan: dirty a known fraction of one table's pages (15%, 25%, 50%), then
# take three incrementals from the same base at combine-distance = 0 / 1M / auto
# and verify each against the two log lines the feature prints:
#
#   pagetracking: test/t1.ibd: <N> changed pages in <R> ranges, typically
#     <G> pages (<B>KB) apart; combine-distance=<D>KB [(auto)] = <M> pages;
#     combined them into <K> reads: ...
...
#   per run:    combine-distance=0     -> nothing merged, and actual == predicted
#               combine-distance=1M -> one logical group (few actual reads)
...
#
# Takes one incremental from full_<label> at --page-tracking-combine-distance=
...
take_and_verify() {
  ...
  local gap_arg=""
  [ "$setting" != "auto" ] && gap_arg="--page-tracking-combine-distance=$setting"
  ...
  vlog "$label combine-distance=$setting: changed=$changed ranges=$ranges" \
  ...
  case "$setting" in
    0)
      ...
      [ "$groups" -eq "$ranges" ] \
          || die "$label: combine-distance=0 must not group: $groups != $ranges"
      [ "$batches" -eq "$groups" ] \
    ...
    1M)
      ...
      [ "$groups" -eq 1 ] \
          || die "$label: combine-distance=1M must form one group, got $groups"
