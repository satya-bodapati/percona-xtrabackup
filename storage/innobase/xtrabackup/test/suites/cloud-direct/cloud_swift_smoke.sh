...
#        * backup_metadata.json is present after backup.
...
grep -q "${PREFIX}/backup_metadata.json" "$TDIR/listing" \
  || die "swift_smoke step 2: ${PREFIX}/backup_metadata.json missing"
...
[ -s "$DL/backup_metadata.json" ] \
  || die "swift_smoke step 3: backup_metadata.json missing in download dir"
[ -f "$DL/sw_test/t1.ibd" ] \
