############################################################################
# xbcloud round-trip against openstackswift/saio (Swift emulator).
#
# Verifies the credential-provider migration for Swift storage
# (KeystoneProvider path).  Skips cleanly when docker isn't
# available.
############################################################################
...
flags=$(cloud_emu_xbcloud_flags swift "$container")

vlog "Full backup → Swift (saio) via xbstream|xbcloud put"
...
vlog "Swift/saio round-trip PASSED (row count $src_count preserved)"
