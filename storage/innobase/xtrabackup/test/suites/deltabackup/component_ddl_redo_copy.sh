###############################################################################
# PXB-XXXX: Classic redo-copy backup with server-journal DDL tracking
#
# With --lock-ddl=REDUCED on a server that provides the backup DDL journal,
# DDL tracking comes from the server journal even for the default
...
# auto must have picked the server DDL journal
if ! egrep -q "DDL tracking source: server DDL journal" $topdir/backup_classic.log ; then
  die "auto ddl-tracking did not resolve to the server DDL journal"
fi
