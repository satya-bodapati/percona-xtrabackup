...
#
# Round trip of --copy-strategy=clone: DML + the full DDL mix (drop/create/rename/
...
xtrabackup_background --backup --target-dir=$topdir/backup_delta \
  --debug-sync-thread="before_file_copy" --lock-ddl=REDUCED --copy-strategy=clone
...
if ! find $topdir/backup_delta/#xb_page_delta -name "*.delta" 2>/dev/null | grep -q . ; then
...
if [ -f "$topdir/backup_delta/#xb_page_delta/test/original_table.ibd.delta" ]; then
