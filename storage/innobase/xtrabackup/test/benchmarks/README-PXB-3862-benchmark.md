...
Defaults: ROWS=10000000, PERCENTS=5:100:5, GAPS=none,0,auto,160K,
...
```bash
VER=1.76.0    # or check https://github.com/datacharmer/dbdeployer/releases
mkdir -p ~/bin
...
build/runtime_output_directory/xtrabackup --help | grep page-tracking-combine-distance
```
...
pxb-bench --workdir $SB run \
    --percents 5:100:5 --gaps none,0,auto,160K --workloads scattered
```

Same densities as the original PXB-3862 chart, so cells overlay it
directly; the `160K` column measures what bridging buys wherever auto
declines.
...
| **10%** | **refused -> 28.4s**               | **bridged -> ~17s = pin-160K** |
| 15-100% | bridged (wins/ties)                | identical |

On fast shared-disk NVMe the 10% verdict is expected to be the
OPPOSITE (refusal, with pin-160K measurably slower) — each machine
