...
static void recv_lazy_read_body(byte *out_buf, const recv_t *recv,
  ...
  /* The lazy path trades one big sequential redo read for many small ones.
  Without these two counters a regression in that trade is invisible. */
  xb_recv_stats.lazy_fetch_calls.fetch_add(1, std::memory_order_relaxed);
  xb_recv_stats.lazy_fetch_bytes.fetch_add(recv->len,
                                           std::memory_order_relaxed);
